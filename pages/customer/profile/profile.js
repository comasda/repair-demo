...
Page({
  ...
  buildView(user) {
    const displayName = user?.profile?.name || user?.username || '客户用户'
    const phone = user?.phone || '未绑定'
    const roleText = roleMap[user?.role] || '客户'
  ...
  ensureAllowedUser() {
    ...
    if (!user || user.isGuest) {
      wx.showToast({ title: '请先登录客户账号', icon: 'none' })
      wx.reLaunch({ url: '/pages/auth/login/login' })
      return null
    }
    if (user.role !== 'customer') {
      wx.showToast({ title: '请使用客户账号查看', icon: 'none' })
      wx.reLaunch({ url: user.role === 'technician' ? '/pages/technician/home/home' : '/pages/customer/home/home' })
      return null
    }
    return user
  },
