// src/pages/technicians/index.tsx
import React, { useEffect, useMemo, useState } from 'react';
import { listTechnicians, TechnicianItem } from '../../services/technicianApi';
...
export default function TechnicianReviewPage() {
  ...
  const [data, setData] = useState<TechnicianItem[]>([]);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await listTechnicians({ status: status || undefined, q: q || undefined, page, pageSize });
      setData(res.items || []);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a' }}>
      ...
      <div style={{
        ...
        <h2 style={{ margin: 0, fontSize: 20 }}>技师审核</h2>
        <label>
          状态：
